...
impl Contract {
    ...
    pub fn get_latest_event(&self) -> BTreeMap<String, String> {
        self.builder
            .query(
                None,
                Key::Account(self.account_addr),
                &["tictactoe_contract".to_string(), "latest_event".to_string()],
            )
            .expect("should be stored value.")
            .as_cl_value()
            .expect("should be cl value.")
...
fn test_deploy() {
    ...
    println!("event: {:?}", contract.get_latest_event());
    ...
    assert_eq!(host_score, U512::from(1));
}
