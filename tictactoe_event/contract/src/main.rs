...
pub const GUEST_SCORE: &str = "guest_score";
pub const HOST_SCORE: &str = "host_score";
...
pub fn check_winner(match_table: [u8; 9]) -> u8 {
    ...
    {
        1
    } else if match_table[0] == match_table[1]
        && match_table[1] == match_table[2]
        && match_table[2] == 2
        || match_table[3] == match_table[4]
            && match_table[4] == match_table[5]
            && match_table[5] == 2
    ...
    {
        2
    } else if match_table.iter().filter(|&e| *e == 0_u8).count() == 9 {
        3
    } else {
        0
    }
...
fn score_change(host: &str, guest: &str, winner: u8) {
    ...
    let mut host_score = dictionary_get::<U512>(dictionary_uref, host)
        .unwrap_or_revert()
        .unwrap_or_default();

    let mut guest_score = dictionary_get::<U512>(dictionary_uref, guest)
        .unwrap_or_revert()
        .unwrap_or_default();
    ...
    match winner {
        1 => {
            host_score += U512::one();
            guest_score = guest_score.saturating_sub(U512::one());
            emit(&T3Event::HostVictory {
                host: host.to_string(),
        ...
        2 => {
            ...
            host_score = host_score.saturating_sub(U512::one());
            emit(&T3Event::GuestVictory {
                host: host.to_string(),
                guest: guest.to_string(),
                host_score,
                guest_score,
        ...
        _ => {
            runtime::revert(T3Error::ImpossibleWinner);
        }
    }
    dictionary_put(dictionary_uref, host, host_score);
    dictionary_put(dictionary_uref, guest, guest_score);
}
